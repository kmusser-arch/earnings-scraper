...
**Step 1 — select the graded metric.** Load `ticker-profiles.json → profiles[TICKER]`. Take the `priority: 1` entry whose `appliesTo == "currentQuarter"`. **That metric, not revenue, is what you grade.** If no profile exists, fall back to revenue and set `confidence: "no-profile"`.
...
| **0.0** | inline, **or a priority-1 hero KPI MISSED → hard cap at 0** (see the cap rule) |
...
### ★ THE HARD CAP RULE — implement this before the band
**A priority-1 hero KPI that MISSES on the current quarter caps Current Quarter at 0.0, regardless of a revenue or EPS beat.** Founding cases: APP Q2 2026 (adj EBITDA margin missed → capped, then pushed to −1.5 by the own-guide failure); CBRS Q2 2026 (hardware revenue −26% vs estimate and −23% YoY → −1.5 despite a +9.9% "core" revenue beat).
...
4. **★ NEVER populate a forward-period KPI from reported actuals.** Classify each `preEarnings.keyKPIs[i].name` by period first:
   ...
   - else → `CURRENT_Q`
   A `CURRENT_Q` parse may only fill a `CURRENT_Q` slot. **Match on the classified period BEFORE matching on the metric word** — matching "revenue" against `FY27 Revenue + EPS Guide` is precisely the bug you hit.
...
```powershell
python validate_library.py    # must exit 0
